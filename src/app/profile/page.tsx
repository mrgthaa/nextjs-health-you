...
const StyledBackground = styled(Box, {
  ...
  '&::-webkit-scrollbar-thumb': {
    backgroundColor: darkMode ? '#FFD70088' : '#ffffff88',
    borderRadius: 6,
  },
  '&::-webkit-scrollbar-thumb:hover': {
    backgroundColor: darkMode ? '#FFD700' : '#fff',
  },
}));
...
const StyledPaper = styled(Paper)(({ theme }) => ({
  ...
  animation: `${fadeIn} 0.6s ease-out`, // ✅ fixed
  width: '100%',
  maxWidth: 600,
  textAlign: 'center',
}));
...
export default function Profile() {
  ...
  const handleSubmit = async () => {
    ...
    const url = editingId ? `${API_URL}/${editingId}` : API_URL; // ✅ fixed backtick
    await fetch(url, {
      method,
  ...
  const handleDelete = async (id: string) => {
    await fetch(`${API_URL}/${id}`, { method: 'DELETE' }); // ✅ fixed backtick
    fetchProfiles();
    setShowForm(true);
  };
      ...
      >
        ...
        >
          {[
            ...
            <MenuItem key={href} onClick={() => setAnchorEl(null)}>
              <NextLink
                href={href}
      ...
      {!showForm && (
        ...
        >
          {profiles.map((profile) => (
            <Box
              key={profile.id}
              sx={{ flex: '1 1 280px', maxWidth: 320 }}
            >
              <ProfileCard>
                <Avatar
