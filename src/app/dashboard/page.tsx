...
export default function Dashboard() {
  ...
  const slideWidth = () =>
    0.8 * (typeof window !== 'undefined' ? window.innerWidth : 320);
  ...
  return (
    <Background darkMode={darkMode}>
      ...
      >
        {ready && upSm && (
          auth ? (
            <Button size="small" sx={btnStyle} onClick={logout}>
              Logout
            </Button>
          ) : (
            <>
        ...
        <Menu
          ...
          PaperProps={{
            sx: {
              background: darkMode ? '#122' : '#2e4c7b',
      ...
      <GlassCard maxWidth="sm" sx={{ mb: { xs: 4, sm: 6 }, width: '100%' }}>
        <GradientTitle variant={upSm ? 'h3' : 'h4'} gutterBottom>
          HealthYou
        </GradientTitle>
        <Typography
          variant="subtitle1"
          sx={{ color: '#eee', fontSize: { xs: '0.9rem', sm: '1rem' } }}
        >
      ...
      <GlassCard maxWidth="xl" sx={{ width: '100%', position: 'relative' }}>
        ...
        {loading ? (
          ...
          >
            {articles.map((a) => (
              <ArticleCard key={a.pageid} {...a} darkMode={darkMode} />
            ))}
          </Box>
        ) : (
          /* slider mobile */
